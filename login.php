...
$sqli = "SELECT * FROM ".$SETTINGS["USERS"]." WHERE `email` = '".mysqli_real_escape_string($connection, $_POST['email'])."' AND `password` = '".mysqli_real_escape_string($connection, $_POST['password'])."'";
		$sqli_result = mysqli_query ($connection, $sqli ) or die ('request "Could not execute SQL query" '.$sqli);
		$user = mysqli_fetch_assoc($sqli_result);
		if(!empty($user)){
